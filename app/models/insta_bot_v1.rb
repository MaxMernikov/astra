  ...
  def self.set_schedule
    InstaUser.where(bot_version: 1).each do |insta_user|
      ...
      when 0        
        ...
        unless InstaSchedule.where(work_type: 'like', run_at: Date.today..(Date.today + 1) ).count > 30
          (rand(4) + 2).times do
            InstaSchedule.create(run_at: InstaSchedule.last_today_at, insta_user_id: insta_user.id, work_type: 'like')
          end

          insta_user.update_columns(bot_step: 1, start_monitoring: Time.now) unless insta_user.start_monitoring
        end
