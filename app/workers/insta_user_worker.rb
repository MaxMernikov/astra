class InstaUserWorker
  ...
  def like
    insta_cache = @insta_user.insta_caches.create(cache_type: 'media', info_hash: $insta_client.user_recent_media(@insta_user.insta_id) )

    sort_media = InstaCache.last.info_hash.sort_by {|media| media.likes[:count]}.reverse
